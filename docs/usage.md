...
| `memory_efficient` | Boolean; `false`                | `genie_g` only; selects the `GENIE_mem` executable, which streams the genotype matrix instead of holding it in memory. It agrees with the in-memory executable on the estimate. It also has a defect the pipeline works around by refusing, not by adjusting: see below.                                                                                                                                                                                                     |
...
`genie_g` reads the cohort's PLINK 1 derivative directly and builds no relatedness matrix. GENIE matches its phenotype and covariate rows **positionally against the FAM** — it reads the identifier columns and discards them — so the pipeline writes one row per FAM row in FAM order. Row order is therefore guaranteed by construction and recorded as a digest in the provenance sidecar; it is not re-verified afterwards, and it cannot be: a covariate file missing one data row, or one whose rows are permuted, both run to completion with every count GENIE echoes still correct and the estimate moved.

What the estimator does assert after the run is that the sample, covariate and per-bin variant counts GENIE echoes match the counts the prepared files declare. That catches a stray header row changing the covariate width, an annotation that disagrees with the variant file, and a retained-sample count the pipeline did not predict — none of which GENIE reports as an error. GENIE parses the header of the phenotype and covariate files rather than skipping it, and the first two fields must be the literal, case-sensitive `FID` and `IID`: a lowercase or differently named header silently changes the estimate, and a header with fewer fields than the data rows segfaults. Missing phenotypes are numeric rather than textual — GENIE drops every sample whose trait parses to exactly `-9`, however it is spelled — so the pipeline applies the same rule and names any sample it affects.
