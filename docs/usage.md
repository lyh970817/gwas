...
| `seed`             | Non-negative integer or `null`  | `genie_g` only; `null` leaves the run unseeded and warned. GENIE treats a negative seed as unseeded and a non-integer seed as `0`, and echoes no seed line at all for an unseeded run, so its estimate cannot be reproduced afterwards.                                                                                                                                                                                                                                      |
...
`genie_g` reads the cohort's PLINK 1 derivative directly and builds no relatedness matrix. GENIE matches its phenotype and covariate rows **positionally against the FAM** — it reads the identifier columns and discards them — so the pipeline writes one row per FAM row in FAM order, and the estimator then asserts the sample, covariate and per-bin variant counts GENIE echoes back against the counts those prepared files declare. That assertion is what catches a positional shift, a stray header row or an annotation that disagrees with the BIM, none of which GENIE reports as an error.
...
> [!WARNING]
> `GENIE_mem` reads past the end of its own per-block buffer when the trailing jackknife block would hold more than twice the nominal block, and dies with `malloc(): corrupted top size` or a segmentation fault and no diagnostic. The pipeline refuses such a combination at preparation and names the alternatives, and reduces its own unset default to the largest count the executable survives. At the scale GENIE is built for the question does not arise — a million variants in a thousand blocks leaves a remainder far below the block size — so this only affects small cohorts. `GENIE` itself is unaffected.

The GENIE gene-by-environment and noise-by-environment models, the `-e`/`-eXannot` environment surface, the `-js` jackknife-scheme flag and the `-tr` trace export are not exposed; `genie.trace` and `genie.environment` are reported as unknown options rather than as operational tuning. `-m G -np 0 -i 1` are fixed by the pipeline: omitting `-m` makes GENIE fit the interaction models instead, and both projection defaults are undocumented in GENIE's own usage text, so the emitted command states them. The GENIE runtime is `linux/amd64` only and has no Conda fallback.
...
| `which GENIE_mem reads past the end of its own buffer`       | Set `genie.jackknife_blocks` to the count the diagnostic names or lower, to a divisor of the cohort's variant count, or set `genie.memory_efficient` to `false`.      |
