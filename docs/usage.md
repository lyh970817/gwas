...
| `heritability_methods`  | By row   | Optional comma-delimited selector: `gcta_greml`, `gcta_greml_ldms`, `ldak_reml`, `ldak_he`, `ldak_pcgc`, `ldak_fast_he`, `ldak_fast_pcgc`, `genie_g`, `mph_reml`, or `mph_reml_ldms`. |
...
| `genie_g`                     | Moment HE (randomised)     | Direct PLINK 1 genotypes | Single            | Yes        | Quantitative only                     | Not consumed                          |
...
Matrix-backed routes reuse a compatible matrix across requests and publish it only under `--save_relatedness_matrices`. A direct-genotype route requests no matrix at all: `ldak_fast_he`, `ldak_fast_pcgc` and `genie_g` read the cohort's PLINK 1 derivative and build nothing under `quality_control/`.

A stochastic estimator randomises rather than enumerating, so two runs of the same input disagree unless the run is seeded. What varies differs by route. `ldak_fast_he`, `ldak_fast_pcgc` and `genie_g` approximate the whole estimate, so both the heritability and its standard error move. `mph_reml` and `mph_reml_ldms` randomise only the trace terms of an otherwise exact REML fit, but that is enough to move their point estimate as well as its standard error. Each of these five routes has a pipeline seed control — `ldak.fast_seed`, `genie.seed` and `mph.seed` — and leaving the LDAK or GENIE one unset keeps the tool's own unseeded default and is warned about, because neither tool records the seed it used and the estimate cannot be reproduced afterwards. `mph.seed` is not warned about, because MPH's own unset default is the fixed seed `0` and it echoes it. For `genie_g` and both MPH routes a seed is necessary but not sufficient: see the option tables below. `ldak_he`, `ldak_pcgc` and `ldak_kvik` are stochastic only in their standard errors — their point estimates are reproducible — and the pipeline exposes no seed option for them yet, so repeated runs of those routes will report slightly different standard errors.
...
Two of the trait-support restrictions have different origins, and the diagnostic reads the same for both. `ldak_pcgc` and `ldak_fast_pcgc` are binary-only because LDAK itself refuses a quantitative trait in those modes. `ldak_fast_he` is quantitative-only because this route never passes a prevalence to LDAK: the tool would happily run a binary trait and return an observed-scale estimate, and reporting that as a heritability without the liability conversion would mislead. `genie_g`, `mph_reml` and `mph_reml_ldms` are quantitative-only for the same reason — neither GENIE nor MPH has a liability-scale conversion at all. Select `ldak_pcgc` or `ldak_fast_pcgc` for a binary trait, or `ldak_he`, which does accept one.
...
`--method_options` is optional. The established form keeps its JSON root keyed by `analysis_id`; each value may contain `gcta`, `genie`, `ldak`, `mph` and/or `regenie`. It remains supported unchanged. A namespaced document places those same entries under `analyses`, summary unary settings under `unary_requests`, and relationship settings under `pair_requests`. Unlisted analysis settings receive their defaults. Every selected LDAK or LDSC summary request must explicitly choose a `reference_bundle_id`; nothing is inferred from the summary's ancestry label.
...
Those three settings define one LD-by-MAF component plan: the LD scores of a cohort's genotype view and the ordered, disjoint SNP groups derived from them. The plan is its own reusable artifact, keyed by the genotype view and the three settings alone, so the GCTA and the MPH stratified families of a row that declares one set of settings share one plan and therefore one SNP membership set instead of partitioning the same variants twice. The settings stay in the `gcta` family because GCTA's own LD-score pass builds the plan: a row selecting only `mph_reml_ldms` still declares `ld_score_region_kb`, `ld_bins` and `ldms_maf_edges` under `gcta`. With `--save_relatedness_matrices` each plan is published once under `quality_control/ldms_component_plans/<plan_key>/`, separately from the matrix families that consume it.
...
| `weights_policy`     | String; `equal`                 | Kinship and direct-genotype routes; `equal` passes `--ignore-weights YES`, `default` passes no weights flag, which LDAK treats identically on both routes because no predictor weights are applied unless `--weights` is given, and `provided` requires `weights`.                                                                                                      |
...
| `fast_num_blocks`    | Integer at least 3 or `null`    | `ldak_fast_he` and `ldak_fast_pcgc` only; `null` keeps LDAK's own 200 blocks. These are **predictor** jackknife blocks — LDAK partitions predictors, not samples — so the setting moves the standard error and not the point estimate. A block-jackknife standard error is not comparable across block settings, nor to the exact `ldak_he`/`ldak_pcgc` standard error. |
| `fast_seed`          | Integer or `null`               | `ldak_fast_he` and `ldak_fast_pcgc` only; `null` leaves the run unseeded and warned. An unseeded LDAK run records no seed anywhere, so its estimate cannot be reproduced afterwards.                                                                                                                                                                                    |
...
| GENIE option       | Type and default                | Consumer and constraints                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------------ | ------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `random_vectors`   | Positive integer or `null`      | `genie_g` only; `null` keeps GENIE's own 10 random vectors and **warns**. Set it explicitly for any published estimate: see the paragraph below.                                                                                                                                                                                                                                                                                                                             |
| `jackknife_blocks` | Integer at least 2 or `null`    | `genie_g` only; `null` uses the lesser of GENIE's own 1000 and the cohort's variant count, because GENIE aborts with a floating-point exception when a block would be empty. A value above the cohort's variant count is **rejected, never clamped**. One block yields a standard error of exactly zero, which is why two is the floor. These are predictor blocks, so the setting moves the standard error and not the point estimate.                                      |
| `seed`             | Non-negative integer or `null`  | `genie_g` only; `null` leaves the run unseeded and warned. GENIE truncates a fractional seed toward zero (`1.5` reproduces `1` byte for byte), parses a non-numeric seed as `0`, and treats a negative integer seed as unseeded while echoing no seed line at all, so an unseeded estimate cannot be reproduced afterwards. All of those are rejected here rather than passed through.                                                                                       |
| `memory_efficient` | Boolean; `false`                | `genie_g` only; selects the `GENIE_mem` executable, which streams the genotype matrix instead of holding it in memory. It agrees with the in-memory executable on the point estimate, but its output is not byte-identical in general — on the compact fixture the enrichment standard error differs at the sixth significant figure — so do not compare the two files directly. It also has a defect the pipeline works around by refusing, not by adjusting: see below.    |
| `annotation`       | Resource path or absent; absent | `genie_g` only; a SNP-by-component 0/1 matrix with one whitespace-delimited row per cohort BIM variant, no header, and **exactly one `1` per row**. Without it GENIE fits the single all-variant component. A row-count disagreement with the BIM, a value other than `0`/`1`, a header row, an unassigned variant and an overlapping assignment are all rejected with a named diagnostic, because GENIE treats every one of them as a warning at most and returns a number. |

`genie_g` reads the cohort's PLINK 1 derivative directly and builds no relatedness matrix. GENIE matches its phenotype and covariate rows **positionally against the FAM** — it reads the identifier columns and discards them — so the pipeline writes one row per FAM row in FAM order. Row order is therefore guaranteed by construction and recorded as a digest in the provenance sidecar; it is not re-verified afterwards, and it cannot be: permuting the covariate rows, or the phenotype rows, runs to completion at exit 0 with every count GENIE echoes still correct and the estimate moved — stably, over eight repeats each.

What the estimator does assert after the run is that the sample, covariate and per-bin variant counts GENIE echoes match the counts the prepared files declare. That catches a stray header row changing the covariate width, an annotation that disagrees with the variant file, and a retained-sample count the pipeline did not predict — none of which GENIE reports as an error. GENIE parses the header of the phenotype and covariate files rather than skipping it, and the first two fields must be the literal, case-sensitive `FID` and `IID`. A lowercase or differently named header is accepted and the fit then becomes non-deterministic: eight repeats on one unchanged body at a fixed seed returned four different values including `-nan`, where the well-formed control was stable across all eight. A header with fewer fields than the data rows segfaults. Missing phenotypes are numeric rather than textual — GENIE drops every sample whose trait parses to exactly `-9`, however it is spelled — so the pipeline applies the same rule and names any sample it affects.

Preparation rejects, with a named diagnostic, several inputs GENIE runs to completion on: a phenotype that is constant across the retained samples (it returns `-nan` variance components at exit 0), a cohort carrying a monomorphic variant (the same), fewer than 30 retained samples (three retained samples returned a heritability of −0.394 with a standard error of 22.06), and a design saturated by its own covariates. Fewer than 1000 retained samples is a warning rather than a refusal. A non-finite estimate is never published: the estimator fails the task instead.

> [!WARNING]
> A seed makes a GENIE fit repeatable, not stable. Measured on a 200-sample cohort at GENIE's native default of ten random vectors, the heritability estimate moved across 0.0719 to 0.1016 over seeds 1 to 8 — about 35% of its own size — while the reported jackknife standard error ranged 0.154 to 0.284 and contains none of that Monte-Carlo component. At 100 random vectors the same spread narrowed to 0.0096. The pipeline keeps `null` meaning GENIE's own default rather than substituting an opinion, and warns; set `genie.random_vectors` explicitly for any estimate you intend to publish.

> [!WARNING]
> `GENIE_mem` reads past the end of its own per-block buffer when the trailing jackknife block would hold more than twice the nominal block, and dies with `malloc(): corrupted top size` or a segmentation fault and no diagnostic. The safe counts are **not** an interval — on a 2200-variant cohort, 274 and 440 run while 300 aborts — so the pipeline refuses such a combination at preparation and names counts it has checked, rather than telling you to lower the value. Its own unset default is reduced to the largest count the executable survives. At the scale GENIE is built for the question does not arise — a million variants in a thousand blocks leaves a remainder far below the block size — so this only affects small cohorts. `GENIE` itself is unaffected.

The GENIE gene-by-environment and noise-by-environment models, the `-e`/`-eXannot` environment surface, the `-js` jackknife-scheme flag and the `-tr` trace export are not exposed; `genie.trace` and `genie.environment` are reported as unknown options rather than as operational tuning.

`-m G -np 0 -i 1` are fixed by the pipeline, in that order, because GENIE's own documentation disagrees with its binary about what those defaults are and because the order itself is load-bearing: `-np` is honoured only when it appears after `-m`, and GENIE echoes no projection line either way, so a reordered command would silently drop the setting with nothing in the output to show it. Its usage text says of `--norm-proj-pheno`: "By default, the phenotype vector is standardized after regressing covariates" — but the measured default is the opposite, and an unset `-np` reproduces `-np 0` byte for byte while `-np 1` moves the reported `Sigma^2_g[0]`. The emitted command therefore states the value actually used instead of inheriting a default the documentation contradicts. Omitting `-m` does not add components — with no environment file GENIE still fits zero gene-by-environment and zero noise-by-environment components — but it silently behaves as `-np 1` and emits a `Total h2_gxe` line for a component the model does not have, which is why the literal is pinned. The GENIE runtime is `linux/amd64` only and has no Conda fallback.

Every LDAK analysis-row estimator — `ldak_kvik`, `ldak_reml`, `ldak_he`, `ldak_pcgc`, `ldak_fast_he` and `ldak_fast_pcgc` — and `genie_g` requires a complete covariate cell for every phenotyped sample. LDAK never parses a missing `--covar` cell: it leaves whatever value was last in its read buffer in place, so the fitted covariate silently becomes the neighbouring column's value for that sample, the previous row's last value, or uninitialised memory. A missing `--factors` cell instead becomes an additional factor level. GENIE's `-c` reader is the same defect in a second tool: one `NA` cell in an otherwise complete file moved the estimate from 0.0925 to 0.0873 at exit 0, with every sample still reported as retained. In none of these cases does the tool drop the sample or say so. The pipeline therefore rejects such an analysis at phenotype preparation and names the offending `FID IID column` cells, rather than reporting a covariate model nobody declared.

| MPH option       | Type and default                  | Consumer and constraints                                                                                                                                                                                                                                                                                                                                                                 |
| ---------------- | --------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `iterations`     | Positive integer or `null`        | `mph_reml` and `mph_reml_ldms` only; `null` keeps MPH's own 20 REML iterations. Exhausting them is a `Warning: not converged after N iterations.` line at exit 0 with a complete, well-formed and materially different result written beside it; the run is not failed on it, so read `classification` in the sidecar, which becomes `estimable_with_warning` and names `not_converged`. |
| `tolerance`      | Number greater than `0` or `null` | `mph_reml` and `mph_reml_ldms` only; `null` keeps MPH's own `0.01` stopping rule on the predicted log-likelihood change. MPH has a native floor near `1e-4`: a smaller value is accepted, echoed in the log and then ignored, so the sidecar records the tolerance you asked for beside the iteration count the solver stopped on.                                                       |
...
| `seed`           | Integer or `null`                 | `mph_reml` and `mph_reml_ldms` only; `null` keeps MPH's own default, which is the fixed integer `0` rather than entropy. The effective value is recorded in the provenance sidecar either way.                                                                                                                                                                                           |
...
An unseeded MPH run is reproducible, which is not true of the other stochastic heritability routes: MPH's unset seed is the fixed integer `0`, not entropy, so repeating a run on one machine at one thread count returns the same estimate. That is why `mph.seed` carries no unseeded-run warning where `ldak.fast_seed` and `genie.seed` do. What a seed does not buy is stability. Any value moves the point estimate itself through the stochastic trace, and the reported standard error carries none of that movement: measured on a 200-sample cohort over seeds 1 to 8, the proportion of variance explained spanned `0.114` at 50 random vectors (`0.022` to `0.137`), `0.052` at MPH's default of 100 (`0.064` to `0.116`) and `0.016` at 1000 (`0.064` to `0.079`), while the reported standard error moved only between `0.124` and `0.129` at that default. Raise `mph.random_vectors` for anything you intend to publish. Two further things move the estimate, both in the sixth to seventh significant digit and neither of them a method option: the thread count the executor gives the task, and `mph.save_memory`. Both are recorded in the per-result provenance sidecar — the thread count read back from MPH's own option echo — so a published MPH estimate is reproducible against the same seed, random-vector count, thread count and memory mode, and not otherwise.
...
MPH's covariate interface shapes what the pipeline hands it in two ways. MPH synthesises an intercept only when no covariate is named at all, so the pipeline always writes an explicit all-ones `intercept` column and names it first; and MPH never expands a categorical covariate, so factor columns are dummy-encoded by the same rule the LDAK matrix-adjustment design uses. When the resulting design is rank deficient MPH prints one warning, prunes columns until it is not, and names none of the columns it dropped — measured, the column it dropped was that explicit `intercept`, which turned a covariate-adjusted fit into a fit through the origin and moved the proportion of variance explained from `0.085` to `0.191`. The route therefore reconciles the covariates named in MPH's own fixed-effect result against the ones it asked for: a pruned `intercept` fails the run, and any other pruned column is named in the sidecar's warnings.

Unlike every LDAK analysis-row estimator and `genie_g`, MPH does not need a complete covariate cell. An empty field is its only missing representation, and a sample carrying one is dropped: MPH reports the reduced set as `Non-missing analysis set contains N individuals`, the pipeline predicts that count before the fit from the cells it wrote, and a disagreement between the two fails the run. What MPH cannot survive is any other spelling of missing — `NA` and non-numeric text abort the process at exit 139, `-9` is silently read as the number minus nine, and a `nan` phenotype sends the solver into an unbounded loop that neither the iteration limit nor the tolerance escapes — so preparation rewrites the pipeline's missing spellings to empty fields and refuses a value that parses as a number but is not finite.
...
| `missing covariate cells`                                                                                                 | Complete the covariate cells the diagnostic names, or drop those samples from the phenotype file. Every LDAK analysis-row estimator and `genie_g` reads a missing cell as a value rather than excluding the sample.                                                                                                        |
...
| `contains a comma or whitespace`, `names repeat` or `which is not a number`                                               | Correct the covariate column the failed `PREPARE_MPH_INPUTS` task names. MPH splits its name lists on commas only, so a name carrying a comma or whitespace addresses a different column or none, and it aborts on a non-numeric cell instead of reporting it. Dummy-encoded factor levels inherit their level's spelling. |
...
### A GENIE analysis was refused before the estimator ran

Several GENIE inputs are checked against the cohort's real genotypes, which manifest validation cannot see, so their diagnostics arrive as a failed `PREPARE_GENIE_INPUTS` task naming the analysis. Each one is a case GENIE itself runs to completion on and returns a number for.

| Diagnostic fragment                                          | Supported action                                                                                                                                                                                                |
| ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `exceeds the ... variant(s) in the cohort`                   | Lower `genie.jackknife_blocks` below the cohort's variant count. GENIE aborts with a floating-point exception when a block would be empty.                                                                      |
| `which GENIE_mem reads past the end of its own buffer`       | Use one of the counts the diagnostic names, any divisor of the cohort's variant count, or set `genie.memory_efficient` to `false`. Lowering the value is not itself a fix: the safe counts are not an interval. |
| `declares ... row(s) but the cohort's BIM declares`          | Give the annotation exactly one row per cohort variant, in BIM order. GENIE reports the mismatch as a warning and then analyses the annotation's own count.                                                     |
| `is in no component column` or `is in ... component columns` | Make the annotation a disjoint partition: exactly one `1` per row. An unassigned variant is silently dropped and an overlapping one makes one label carry two values.                                           |
| `carries the value ...; every cell must be 0 or 1`           | Remove the header row or the non-binary cell. GENIE reads either as no component at all and returns `-nan`.                                                                                                     |
| `the phenotype is constant`                                  | The trait has no variance across the retained samples; GENIE returns `-nan` variance components at exit 0.                                                                                                      |
| `below the pipeline's floor of 30`                           | Too few samples survive alignment for a randomised fit. GENIE returns a finite, meaningless estimate rather than failing.                                                                                       |
| `are monomorphic across the FAM`                             | Filter the named variants out of the cohort genotypes. One monomorphic variant takes the whole fit to zero variance components and `-nan`.                                                                      |
| `have no row at all in the covariate design`                 | Give every phenotyped sample a covariate row, or drop it from the phenotype file. GENIE reads the covariate file positionally and has no covariate-missing code.                                                |
