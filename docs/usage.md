...
| `ldak_fast_he`                | Moment HE (randomised)     | Direct PLINK 1 genotypes | Single            | Yes        | Quantitative only                     | Not consumed                          |
...
A stochastic estimator approximates its answer by randomised sampling, so two runs of the same input disagree unless the run is seeded. Every stochastic family exposes a seed option (`ldak.fast_seed` for the direct-genotype LDAK routes); leaving it unset keeps the tool's own unseeded default and the pipeline warns that the estimate cannot be reproduced. The component model names the variance-component structure the estimator fits, and is what decides whether the LD- and MAF-stratified plan settings apply to a row.
...
Every LDAK analysis-row estimator — `ldak_kvik`, `ldak_reml`, `ldak_he`, `ldak_pcgc`, `ldak_fast_he` and `ldak_fast_pcgc` — requires a complete covariate cell for every phenotyped sample. LDAK reads a missing `--covar` cell as the number it parses to and turns a missing `--factors` cell into an additional factor level; in neither case does it drop the sample or say so. The pipeline therefore rejects such an analysis at phenotype preparation and names the offending `FID IID column` cells, rather than reporting a covariate model nobody declared.
