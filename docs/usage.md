...
`gcta_bivariate_reml_ldms` additionally publishes a derived genome-wide genetic correlation beside its native files, because GCTA's multi-GRM bivariate REML reports only per-stratum ones; see the [output documentation](output.md) for what it is and when its standard error is not a confidence statement. Your `--reml-no-constrain` and `--reml-maxit` choices remain yours, are rendered into the fit unchanged, and are recorded in `derived.provenance.json`. On a small cohort a multi-stratum bivariate fit often reaches the constrain boundary and exhausts the iteration cap under GCTA's constrained default; `--reml-no-constrain` is what makes such a fit converge, and it changes what the result is.
...
An unseeded MPH run is reproducible, which is not true of the other stochastic heritability routes: MPH's unset seed is the fixed integer `0`, not entropy, so repeating a run on one machine at one thread count returns the same estimate. That is why `mph.seed` carries no unseeded-run warning where `ldak.fast_seed` does. What a seed does not buy is stability. Any value moves the point estimate itself through the stochastic trace ([#67](https://github.com/lyh970817/gwas/issues/67)), and the reported standard error carries none of that movement: measured on a 200-sample cohort over seeds 1 to 8, the proportion of variance explained spanned `0.114` at 50 random vectors (`0.022` to `0.137`), `0.052` at MPH's default of 100 (`0.064` to `0.116`) and `0.016` at 1000 (`0.064` to `0.079`), while the reported standard error moved only between `0.124` and `0.129` at that default. Raise `mph.random_vectors` for anything you intend to publish. Two further things move the estimate, both in the sixth to seventh significant digit and neither of them a method option ([#68](https://github.com/lyh970817/gwas/issues/68)): the thread count the executor gives the task, and `mph.save_memory`. Both are recorded in the per-result provenance sidecar — the thread count read back from MPH's own option echo — so a published MPH estimate is reproducible against the same seed, random-vector count, thread count and memory mode, and not otherwise.
...
| Which samples are non-missing | REGENIE mean-imputes missing observations across the whole invocation, so an analysis batched with a differently-missing sibling would be fitted against a sample set it would not have alone. |
