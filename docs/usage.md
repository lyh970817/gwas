...
Treat both routes as a recommended REML candidate rather than an established default. Until parity with the GCTA pair routes has been demonstrated on data you recognise — which means a complete pair, where the two estimators fit the same individuals — read an MPH pair estimate as a second opinion rather than as the primary result.
