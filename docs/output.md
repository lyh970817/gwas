...
Pairwise outputs instead use the deterministic request ID `<method>--<relationship_id>`. Find `relationship_id` in `--relationship_manifest`, follow its ordered left and right endpoint IDs, and inspect the native result and log under `requests/<method>/<request_id>/`. The pipeline does not add a normalized estimand table or a diagnostics table, and never rewrites a native file. It publishes each native result unchanged and adds a per-result invocation record beside it for a route whose runtime does not record its own invocation, as the four MPH routes do not: MPH reports its quality failures as warnings at exit 0 and never restates the settings it fitted under. Anything the pipeline computes for itself is published beside the native files under a `derived.` prefix, states `derived` in its own columns, and carries its own provenance. Exactly one route does that today: `gcta_bivariate_reml_ldms`, whose native result carries no genome-wide genetic correlation.
...
- `requests/gcta_bivariate_reml_ldms/<request_id>/`
  - `native.hsq`: Complete native GCTA bivariate REML-LDMS result: one `V(Gk)_tr1`, `V(Gk)_tr2` and `C(Gk)_tr12` row per stratum, the residual rows, the phenotypic variances, and one genetic correlation `rGk` per stratum. There is no genome-wide total row; GCTA does not compute one for this model ([#34](https://github.com/lyh970817/gwas/issues/34)).
  ...
  - `derived.total_rg.tsv`: The pipeline's own genome-wide total, in the primary `component = total` row (`origin = delta_method`), with the native per-stratum values repeated as `G1..GK` rows (`origin = native`) as secondary diagnostics. Columns: `request_id`, `relationship_id`, `method`, `component`, `left_analysis_id`, `right_analysis_id`, `left_trait_id`, `right_trait_id`, `scale`, `left_variance`, `left_variance_se`, `right_variance`, `right_variance_se`, `covariance`, `covariance_se`, `rg`, `rg_se`, `origin`, `n_components`. `scale = observed` qualifies the variance and covariance columns only; `rg` and `rg_se` are scale-invariant and need no liability conversion even when `--reml-bivar-prevalence` was passed. A derived value the fit does not support is `NA`, never a number: a non-positive summed genetic variance leaves the whole `total` row `NA`, and a non-positive or non-finite sampling variance leaves that one standard error `NA`. The `G1..GK` rows are GCTA's own text character for character, so a value GCTA itself printed as `-nan` is reproduced as it stands and named in the provenance's `warnings` rather than rewritten.
  - `derived.provenance.json`: Record of that derivation: the parameter order parsed from the native files, the complete sampling covariance matrix and the file it was read from, the maximum deviation of the diagonal cross-check, the per-stratum cross-check record, the number of constrained components, whether GCTA reported bending a variance-covariance matrix to positive-definiteness, the residual-covariance decision with the native sentence or option token it was read from, the declared prevalence, the rendered native arguments, and the classification with its warnings.
...
`gcta_bivariate_reml_ldms` is the one route where the pipeline computes a number GCTA does not. A multi-component model's primary result is the genome-wide genetic correlation, and GCTA's multi-GRM bivariate REML reports only a per-stratum one; the total is an unimplemented feature there rather than a numerical fault, and the same package's HEreg estimator prints a total for the same model ([#34](https://github.com/lyh970817/gwas/issues/34)). The pipeline therefore derives

```text
sum(C(Gk)_tr12) / sqrt(sum(V(Gk)_tr1) * sum(V(Gk)_tr2))
```

with a delta-method standard error over all `3K` genetic parameters, taken from the complete sampling variance/covariance matrix in `native.log`. It is never built from the per-stratum standard errors alone, which would ignore every cross-stratum covariance. A one-stratum plan is the degenerate case, where the total reproduces the single component's native `rG`. Read four things about it before using the number:

- The derived value is never presented as GCTA output. It lives in its own `derived.*` files, carries `origin = delta_method` in the table, and the native `.hsq` and `.log` beside it are republished byte for byte. Rows whose `origin` is `native` are GCTA's own text.
- The delta-method standard error is asymptotically valid for an interior solution. When GCTA holds a component at its constrain boundary, or reports that it bent a variance-covariance matrix to positive-definiteness, the provenance carries `standard_error_unreliable_constrained_fit` or `sampling_covariance_bended_se_unreliable` and the classification drops to `estimable_with_warning`. Do not read that standard error as a confidence statement.
- `rg` and `rg_se` are scale-invariant, and `scale = observed` qualifies only the variance and covariance columns. A prevalence-declared request needs no liability conversion of the correlation; the declared prevalence is recorded in the provenance so the choice is auditable.
- The derivation runs after the native fit has already succeeded and been published. A data outcome of that fit — a non-positive summed genetic variance, a non-positive sampling variance, a non-finite native value — is classified and named in `warnings` with `NA` in the affected column, and never fails the run. The run fails only when the native output's structure contradicts the layout this parser reads, which would mean any derived number was untrustworthy.
...
The genome-wide total is MPH's own number. MPH prints the genome-wide genetic correlation and its standard error itself, as the synthetic `G` row of `native.mq.cor.csv`, for the one-component and the multi-component fit alike; for `mph_bivariate_reml` that row is the single component's row. The pipeline publishes what MPH wrote and derives nothing, so these two routes have no derived estimand table at all; the sidecar's `correlations` block repeats MPH's own component, residual and total values rather than recomputing them. That is the difference from `gcta_bivariate_reml_ldms`, whose native output carries no total genetic correlation ([#34](https://github.com/lyh970817/gwas/issues/34)) and which therefore publishes a derived one in a separate `derived.total_rg.tsv`. The two totals are not interchangeable: MPH's is its own estimate and GCTA's is the pipeline's delta-method construction over GCTA's per-stratum components, and MPH fits the complete-case intersection of the pair where GCTA's bivariate REML fits an unbalanced design over the same two endpoints.
