...
def validateGenotypeGroup(cells, reject) {
    ...
    if (!populated_groups) {
        ...
        groups[group_name].findAll { column -> !cells[column] }.each { column ->
    ...
    rows.eachWithIndex { row, index ->
        ...
        validated_rows << [
            meta + [
                ...
                is_binary: is_binary,
                // Case and control source values are carried as strings: real phenotype files use
                // numeric and textual conventions alike, and nothing downstream compares them
                // numerically. nf-schema infers a type from the raw cell before this pass sees it,
                // so a value it read as a number is spelled back out here rather than preserved
                // byte for byte — `2` survives as "2" and `0010` as "0010", but an exponent form
                // such as `1E2` would be normalised.
