...
workflow PLINK_GWAS_REGENIE {
    ...
    ch_step2 = ch_step2_genotypes
        ...
        .groupTuple(by: 0)
        .join(PLINK_FIT_REGENIE.out.predictions.map { meta, predictions -> tuple(meta.id, tuple(meta, predictions)) }, by: 0, failOnDuplicate: true, failOnMismatch: true)
        .join(PLINK_FIT_REGENIE.out.loco.map { meta, loco -> tuple(meta.id, tuple(meta, loco)) }, by: 0, failOnDuplicate: true, failOnMismatch: true)
        ...
        .join(ch_step2_bsize.map { meta, step2_bsize -> tuple(meta.id, step2_bsize) }, by: 0, failOnDuplicate: true, failOnMismatch: true)
        .flatMap { _analysis_id, genotype_shards, predictions, loco, pheno, covar, step2_bsize ->
            genotype_shards.collect { genotype_shard ->
                tuple(tuple(predictions[0], genotype_shard[1], genotype_shard[2], genotype_shard[3]), tuple(predictions[0], predictions[1], loco[1]), pheno, covar, step2_bsize)
            }
        }
